  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'login') {
        ...
        showMessage('success', data.message + (data.token ? ' (token received)' : ''));
      } else {
        const data = await register(name, email, password);
        showMessage('success', data.message + (data.user ? ` — ${data.user.email}` : ''));
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm rounded-2xl bg-slate-900/80 border border-slate-700/60 shadow-xl p-8">
        ...
        <p className="mt-6 text-center text-xs text-slate-500">
          Backend: ensure it’s running on port 5000 for /auth proxy.
        </p>
      </div>
    </div>
  );
