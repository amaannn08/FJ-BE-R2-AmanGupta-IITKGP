  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-700">Income vs expenses report</h2>
        {onClose && (
          <button
            type="button"
            onClick={onClose}
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="px-3 py-2 font-medium text-slate-600">Period</th>
                <th className="px-3 py-2 font-medium text-slate-600 text-right">Income</th>
                <th className="px-3 py-2 font-medium text-slate-600 text-right">Expense</th>
                <th className="px-3 py-2 font-medium text-slate-600 text-right">Net</th>
              </tr>
            </thead>
            <tbody>
              {result.data.map((row) => (
                <tr key={row.period} className="border-b border-slate-100">
                  <td className="px-3 py-2 text-slate-800">{formatPeriod(row.period)}</td>
                  ...
                  <td className="px-3 py-2 text-right text-slate-700">{formatCurrency(row.net ?? row.income - row.expense)}</td>
                </tr>
              ))}
            </tbody>
          </table>
